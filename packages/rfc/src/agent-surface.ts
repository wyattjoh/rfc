...
needs_split is an instruction, not a failure. The result carries a subQuestions array naming each atomic question the request bundled, plus the RFC and the review candidates already retrieved. Research each listed sub-question in its own call against the RFC the result already names, then answer every part; the one-follow-up budget applies per sub-question. Never rephrase and retry the compound request, never re-research an RFC this session already researched, and never refuse the whole request when the result already carries usable material.
