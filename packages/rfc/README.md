...
Run `rfc costs` to read those cumulative global totals without changing them. JSON output uses the same validated `rfc_usage_totals` document stored on disk. `rfc costs --format human` prints the update timestamp, operation and token coverage, and estimated USD total. When the usage file does not exist yet, the command reports zero totals and does not create it.
